...
class ModrinthClient:
    ...
    def _filter_versions(versions: List[Dict[str, Any]], mc_version: str, mod_loader: str) -> List[Dict[str, Any]]:
        ...
        return [
            v for v in versions
            if (mod_loader in [l.lower() for l in v.get("loaders", [])] and
...
class ModManager:
    ...
    def display_mod_status(self, mod_info: Dict[str, Any], versions: List[Dict[str, Any]],
                          current_version_id: str, update_mode: bool = False) -> Tuple[bool, int]:
        ...
        if not versions:
            print(f"Status: No compatible version found")
            print("-" * 80)
            return False, 0
        ...
        if needs_update:
            updates_performed = self._handle_update_needed(
                current_version, current_version_id, latest_version, 
                mod_id, mod_title, update_mode
            )
        else:
    ...
    def _display_version_info(self, current_version: Optional[Dict[str, Any]], 
                             current_version_id: str, latest_version: Dict[str, Any]) -> None:
    ...
    def _handle_update_needed(self, current_version: Optional[Dict[str, Any]], 
                             current_version_id: str, latest_version: Dict[str, Any],
                             mod_id: str, mod_title: str, update_mode: bool) -> int:
        ...
        if update_mode:
            return self._perform_update(
                mod_id, mod_title, current_version_id, 
                latest_version, color
            )
        return 0
    
    def _perform_update(self, mod_id: str, mod_title: str, 
                       current_version_id: str, latest_version: Dict[str, Any],
                       color: str) -> int:
    ...
    def _should_update_to_slug(self, mod_id: str, slug: str) -> bool:
        ...
        return self.prompt_yes_no(
            f"Do you want to use the readable slug '{slug}' instead of ID '{mod_id}'?"
        )
    ...
    def check_mods(self, update_mode: bool = False, specific_mods: Optional[List[str]] = None) -> Tuple[List[str], int]:
        ...
        for mod in self.config.get("mods", []):
            ...
            mod_needs_update, mod_updated = self.display_mod_status(
                mod_info, versions, current_version_id, update_mode
            )
...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument('--update', nargs='*', metavar='MOD_ID',
                      help='Update specified mods to latest versions (or all if no mods specified)')
    group.add_argument('--slugs', action='store_true',
                      help='Convert mod IDs to readable slugs')
    parser.add_argument('--file', default='server.toml',
...
def handle_mod_updates(manager: ModManager, config: Dict[str, Any], 
                      mc_version: str, mod_loader: str, file_path: str,
...
def display_update_summary(update_mode: bool, updates_performed: int, 
                          file_path: str, needs_update: List[str]) -> None:
